...
from .base import sqrtm
from .geodesic import geodesic
from .test import is_sym_pos_semi_def as is_spsd


def _limit_pdf(y, xi):
    """
    Implements Equation (15) from Said et al. 
    "Riemannian Statistics Meets Random Matrix Theory: 
    Toward Learning From High-Dimensional Covariance Matrices" (2022).
    
    """
    # Term inside the square root in eq (15)
    term_inside_sqrt = 4 * np.exp(xi) * y - (y + 1)**2
    
    #print(term_inside_sqrt)
    if term_inside_sqrt > -1e-6:
        term_inside_sqrt = np.abs(term_inside_sqrt)
        
    assert term_inside_sqrt > 0, "_limit_pdf failed, term inside sqrt is negative"
        
    numerator = np.sqrt(term_inside_sqrt)
    denominator = y + 1
    
    density = (1.0 / (np.pi * xi * y)) * np.arctan(numerator / denominator)
    return density

def sample_eigenvalues_from_limit_pdf(N, sigma):
    """
    Generates eigenvalues using the limit distribution from Proposition 3 from 
    Said et al. "Riemannian Statistics Meets Random Matrix Theory: 
    Toward Learning From High-Dimensional Covariance Matrices" (2022).
    Replaces the Monte Carlo procedure for a large N.

    Args:
        N (int): Dimension of the matrices being sample
        sigma (float): Dispersion of the Riemannian Gaussian distribution.

    Returns:
        log(y): The log of the eigenvalues
    """
    
    t = N * sigma**2
    xi = 1 * t / 2.0
    
    # Calculate bounds a(xi) and b(xi), a(xi) <= y <= b(xi)
    c = np.exp(-xi)
    sqrt_term = np.sqrt(1 - c)
    a = c * (1 + sqrt_term)**(-2)
    b = c * (1 - sqrt_term)**(-2)
    
    # Compute the Cumulative Distribution Function (CDF)
    y_grid = np.linspace(a, b, 1000)
    pdf_vals = np.array([_limit_pdf(y, xi) for y in y_grid])
    cdf_vals = np.cumsum(pdf_vals)
    cdf_vals /= cdf_vals[-1] # Normalize to 0-1
    
    # In the large N limit, the eigenvalues are static and evenly spaced.
    target_quantiles = (np.arange(1, N + 1) - 0.5) / N
    
    # Compute
    inverse_cdf = interp1d(cdf_vals, y_grid, kind='linear', bounds_error=False, fill_value=(a, b))
    y_samples = inverse_cdf(target_quantiles)
    
    return np.log(y_samples)
...
def _pdf_r(r, sigma):
    ...
    """ -- Vectorized version -- """
    # Compute all pairwise absolute differences divided by 2
    diff = np.abs(r[:, None] - r[None, :]) / 2
    
    # Take only the upper triangle (i < j)
    upt_diff = np.triu_indices(len(r), k=1)
    
    partial_2 = np.sum(np.log(np.sinh(diff[upt_diff])))
...
def _slice_one_sample(ptarget, x0, w, rs):
    ...
    for i in range(n_dim):
        ...
        # step 4 : loop (BIGGEST BOTTLENECK)
        while True:
            xprime_i = xl_i + (xr_i - xl_i) * rs.rand()
            Px = ptarget(xt + (xprime_i - xt[i]) * ei)
            #print(Px, uprime_i, n_dim)
            """
            When N>20 both Px and uprime_i become too close to zero,
...
def _sample_parameter_r(n_samples, n_dim, sigma,
    ...
    .. [1] `Riemannian Gaussian distributions on the space of symmetric
        ...
        [2] `Gaussian distributions on Riemannian symmetric spaces in the large N limit`
            <https://doi.org/10.48550/arXiv.2102.07556>
            Simon Heuveline, Salem Said, Cyrus Mostajeran. 2021
    """
    ...
    # If n_dim is sufficiently large, we get the eigenvalues from the limit distribution.   
    if n_dim > 7:
        r_single = sample_eigenvalues_from_limit_pdf(n_dim, sigma)
        # The same eigenvalues are used for every sample. This holds true according to [2]. 
        r_samples = np.tile(r_single, (n_samples, 1))
