...
class Diffusion:
    ...
    def sample(self, dataset_center_path, model,n):
        ...
        with torch.no_grad():
            ...
            x = sample_gaussian_spd(n,mean,1,n_jobs=-1)
            x = torch.tensor(x).to(self.device)
            test_dis = spd_dis(init_tensor,x).mean().cpu()
            #print(test_dis)
            pbar = tqdm(reversed(range(1,self.noise_steps)), total=self.noise_steps-1)
            for i in pbar:
                ...
                pbar.set_description(f"Loss: {loss:.4f}, Test Distance: {test_dis:.4f}, Distance to Identity Matrix: {dis2:.4f}")
...
def launch():
    ...
    parser.add_argument("--spd_size", type=int, default=8)
    parser.add_argument("--time_size", type=int, default=256)
    parser.add_argument("--dataset_path", type=str, default="data/uncondition/train_data.csv")
    parser.add_argument("--dataset_center_path", type=str, default="data/uncondition/exp1_setting1_init.csv")
    parser.add_argument("--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu")
    parser.add_argument("--lr", type=float, default=0.0015)
    parser.add_argument("--results_dir", type=str, default= 'result/spd_ddpm_un-' + datetime.now().strftime("%Y-%m-%d-%H-%M-%S"))
    ...
    if args.dataset_name != "synthetic":
        args.batch_size = 4
        args.spd_size = 100
        args.dataset_path = "data/fmri/adhd_schaefer_dataset.pt"
